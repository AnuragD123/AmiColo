...
export default function FindRoomPageLayout({ children }) {
  ...
  return (
    <div className='flex flex-row gap-2 w-full h-full relative'>
      {/* <div className={`w-72 bg-gradient-to-r from-cyan-200 to-blue-200  px-5 relative py-8 transition-all duration-300 ease-linear ${sidebartoggle ? "max-sm:w-60 w" : "max-sm:w-0"} max-sm:fixed top-0 h-full max-sm:px-0 z-10`} style={{ backgroundImage: "linear-gradient(to right, #BBEEE0 , #B6D0FE)" }}>
      ...
      <div className="w-4/5 mx-auto mt-10 max-md:w-3/5 max-sm:w-full max-sm:mx-4 ">
        {children}
