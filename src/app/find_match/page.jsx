...
import { handleSanitizeData } from "@/helper";
...
const FindMatch = () => {
  ...
  const currentMatch = users.slice(indexOfFirstUser, indexOfLastUser);
  ...
  return (
    <>
      ...
      <div className=" flex-1 p-2 flex flex-col items-center">
        <p
          className="text-center w-full text-3xl text-white font-extrabold py-3"
          style={{
        ...
        <div className="mt-5 w-full grid md:grid-cols-2 lg:grid-cols-3 gap-4 justify-center items-center">
          {currentMatch.map((user) => (
            <Roomate key={user.id} data={handleSanitizeData(data)} />
          ))}
        </div>
          ...
          {users.length > postPerPage && (
            <ul className="flex flex-wrap items-center gap-4">
              <button
                ...
                className="bg-blue-600 bg-opacity-70 hover:bg-opacity-100 transition-all duration-300 ease-linear text-white p-3 rounded-full"
              >
                <GrPrevious />
              </button>
              {getPageNumbers().map((number, index) => (
                <li key={index} className="">
                  <button
                    ...
                    className={
                      currentPage === number
                        ? " text-white bg-blue-600 rounded-full px-4 py-2"
                        : ""
                    }
                    disabled={number === "..."}
                  >
